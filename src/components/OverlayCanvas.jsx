 ...
 */
export default function OverlayCanvas({ geometry, transform, width, height, canvasRef, renderRef }) {
  ...
  useEffect(() => {
    ...
    // Expose a forceRender so captureFrame can re-draw right before toDataURL
    if (renderRef) {
      renderRef.current = {
        forceRender: () => {
